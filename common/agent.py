...
def analyse_issue(project: Project, issue_iid: int) -> str:
    ...
    for iteration in range(settings.agent_max_iterations):
        ...
        is_last = iteration == settings.agent_max_iterations - 1
        if is_last:
            messages.append({
                "role": "user",
                "content": "You have reached the tool call limit. Write your final analysis now using only what you have gathered so far. Do not call any more tools.",
            })
        response = llm.chat(messages, tools=None if is_last else TOOLS)
